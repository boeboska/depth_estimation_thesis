...
class ASP_OC_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        feat2 = self.conv2(x) # 1, 256, 24, 80
        feat3 = self.conv3(x) # 1, 256, 24, 80
        feat4 = self.conv4(x) # 1, 256, 24, 80
        feat5 = self.conv5(x) # 1, 256, 24, 80

        # true
        if isinstance(x, Variable):
            out = torch.cat((feat1, feat2, feat3, feat4, feat5), 1) # 1, 1280, 24, 80
        ...
        output = self.conv_bn_dropout(out) # 1, 512, 24, 80
